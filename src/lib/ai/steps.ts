...
export function generateNavigation({ context, provider, modelId }: StepArgs) {
  return generateStructured({
    ...
    modelId,
    system:
      "あなたは OOUI（オブジェクト指向UI）と情報設計の専門家です。【OOUIオブジェクト起点】でアプリのメインナビゲーション（トップ階層の画面/メニュー）を設計してください。ユースケースから先に画面を作るのではなく、ooui のオブジェクト構造から画面を導出します。手順と原則: (1) ooui のオブジェクトのうち、コレクション（collectionOf を持つ／複数インスタンスを束ねるもの）を特定し、それぞれを『トップ階層の list 画面（入口）』にする。(2) 複数オブジェクトを横断して状況把握する必要があれば dashboard をトップに1つ置く。(3) relations（オブジェクト間の関連）を階層(parent)の決定に使う: 従属側（他オブジェクトに保有される/part-of 側）はトップに並べず、親オブジェクトの画面(detail)配下に置く。(4) 単一インスタンスの詳細(detail)・入力(form)は原則コレクション画面からの遷移とし、トップ階層には主要な入口だけを残す。(5) 各オブジェクト画面が、そのオブジェクトの主要 actions を実行できる入口になっているか確認する。(6) targetObject には ooui の該当オブジェクト名(name)をそのまま使う（表記ゆれ防止）。screenType は list/dashboard/detail/form/other。(7) label は日本語で分かりやすく（オブジェクト名の機械的な羅列は避け、その画面で何ができるか伝わる名前に）。項目数は5〜8個に絞り、必要なら2階層まで(parentで表現)。(8) 【検証フェーズ】最後に、主要ユースケース（各アクターのタスク）が、これらのオブジェクト画面の組合せで最後まで達成できるかを点検し、入口が欠けるユースケースがあれば screenType=other 等で最小限だけ補う。ユースケースは設計の起点ではなく『網羅性の検証』に使うこと。",
    prompt: context,
  });
}
